...
/**
 * Wire schema of the `turnModels` view: `"turn:step" -> model id`.
 *
 * DSH 0.1.5 的 `ProjectionDefinition` 只认 `stateSchema` 和 `wire.{viewSchema,view}`；
 * 旧的顶层 `schema` / `view` 会被 `register()` 原样丢弃（register 只搬运
 * `definition.wire`，见 dsh-session-projection/lib/index.js:68-80），
 * 结果是单元注册成功但 `def.wire === undefined`，`snapshot()` 直接跳过它
 * （同文件 :147），浏览器永远读不到 `turnModels`。所以三个字段名都不能改回来。
 */
const turnModelsViewSchema = z.record(z.string(), z.string());
/** Pure fold + view for per-step model attribution. */
const turnModelsProjectionDefinition = {
	/** Projection key the browser half reads. */
	key: "turnModels",
	/** Host fold state is the same plain-JSON shape as the wire payload. */
	stateSchema: turnModelsViewSchema,
	init: () => ({}),
	apply: (state, event) => {
	...
	/**
	 * `wire` 让该单元成为 client-visible 投影。`view` 返回 state 本身：drive 用
	 * `Object.is` 比较两次 view 结果来抑制无变化发布，而 apply 不变就返回同一
	 * 引用，因此不会产生多余帧。
	 */
	wire: {
		viewSchema: turnModelsViewSchema,
		view: (state) => state
	},
	stateVersion: 1
};
...
// 成本口径版本与一次性迁移（改动前先读）
// ────────────────────────────────────────────────────────────────
// v1 = 成本按【人民币】计价（官方定价页给的就是人民币单价）。
// v2 = 成本按【美元】计价（插件面向按美元计费的账号）。
// 换算只在落盘前做一次：读到的 v1 记录 ÷ USD_CNY_RATE 并升到 v2，
// 之后任何路径都不再换算。这样重复迁移是幂等的 —— v2 不会被二次相除。
/** 当前成本口径版本；与 lib/client.js 发送的 `v` 必须一致。 */
const COST_VERSION = 2;
/**
 * 人民币/美元汇率（1 美元 = 多少元）的兜底值。
 *
 * 实际取值为「可手动覆盖」+「启动时自动获取」+「回退到这里」三级：
 * 1. 环境变量 `DSH_USAGE_USD_CNY_RATE`（想固定汇率时用）
 * 2. 启动时向公开汇率接口取一次（见 {@link initExchangeRate}）
 * 3. 取不到就用这个兜底值（2026-09-09 中间价）
 *
 * 为什么启动取一次而不是每次计算都取：历史记录里存的是换算后的美元值，
 * 同一批数据必须用同一个汇率换算，否则数字会前后不一致。启动取一次能保证
 * 单次运行内汇率恒定，同时不需要你手动追汇率。
 */
const DEFAULT_USD_CNY_RATE = 6.7769;
/** 当前生效的汇率（启动时确定；见 DEFAULT_USD_CNY_RATE 的说明）。 */
let USD_CNY_RATE = DEFAULT_USD_CNY_RATE;
/** 汇率取值范围（人民币/美元），超出视为接口异常，拒绝采用。 */
const USD_CNY_RATE_MIN = 5;
const USD_CNY_RATE_MAX = 9;

/**
 * 把一条记录的成本从人民币口径升到美元口径（幂等）。
 * v2 及以上的记录原样返回；缺失 `v` 的历史行按 v1 处理。
 * @param record - 已解析的记录（含 cost / v）。
 * @returns 升到 {@link COST_VERSION} 后的记录；已是当前版本时返回同一引用。
 */
function migrateCost(record) {
	if (record.v >= COST_VERSION) return record;
	return { ...record, v: COST_VERSION, cost: record.cost / USD_CNY_RATE };
}
...
function parseRecord(raw) {
	...
	return migrateCost({
		// 缺失 `v` 的历史行按 v1（人民币）处理，由 migrateCost 升到当前口径。
...
/**
 * 把一行人民币单价换算成美元单价。与 lib/client.js 的 usdRow 是同一口径，
 * 两边必须一致：宿主用它重算 cost（repairWeekendPeak / repairUnknownModels），
 * 客户端用它给新记录定价。宿主漏掉这一处就会用人民币价重算美元存量，
 * 金额被放大整整一个汇率（实测 6.7769 倍）——这个坑踩过一次，别再踩。
...
/**
 * 确定本次运行使用的汇率：环境变量优先，其次启动时自动获取，最后回退兜底值。
 *
 * 必须在处理任何历史记录之前完成 —— 存量迁移与价格表都依赖它，
 * 中途改汇率会让同一批数据出现两种换算结果。取不到（离线/接口异常/
 * 返回值离谱）一律静默回退，绝不让网络问题影响插件启动。
 * @param log - 可选的日志函数（宿主 ctx.logger.info）。
 * @returns 实际生效的汇率。
 */
async function initExchangeRate(log) {
	// 1) 手动覆盖：想固定汇率时设这个环境变量。
	const override = Number(process.env.DSH_USAGE_USD_CNY_RATE);
	if (Number.isFinite(override) && override >= USD_CNY_RATE_MIN && override <= USD_CNY_RATE_MAX) {
		USD_CNY_RATE = override;
		log?.(`ui-usage: 汇率来自环境变量 DSH_USAGE_USD_CNY_RATE = ${override}`);
		return USD_CNY_RATE;
	}
	// 2) 自动获取（无需 API key；任一处失败即回退）。
	try {
		...
		if (response.ok) {
			...
			if (Number.isFinite(rate) && rate >= USD_CNY_RATE_MIN && rate <= USD_CNY_RATE_MAX) {
				USD_CNY_RATE = rate;
				log?.(`ui-usage: 汇率自动获取 USD->CNY = ${rate}`);
				return USD_CNY_RATE;
			}
			log?.(`ui-usage: 汇率为异常值 ${String(body?.rates?.CNY)}，回退 ${DEFAULT_USD_CNY_RATE}`);
		}
	} catch (error) {
		log?.(`ui-usage: 汇率获取失败（${error instanceof Error ? error.message : String(error)}），回退 ${DEFAULT_USD_CNY_RATE}`);
	}
	// 3) 回退：保持 DEFAULT_USD_CNY_RATE（USD_CNY_RATE 初值）。
...
/**
 * 一次性把存量历史从人民币口径迁移到美元口径。
 *
 * 幂等的关键：{@link parseRecord} 读文件时已经对 v1 行做过除法并升级，
 * 因此 `loadRecords` 出来的记录都是当前版本。这里再比一次原始行里的 `v`，
 * 只有在确实存在 v1 行时才重写文件 —— 迁移过一次之后 `v` 全是 2，
 * 这个函数直接返回 0，绝不会把金额二次相除。
 *
 * 检测的是原始字节里的版本号，不能用 loadRecords 的结果判断，否则
 * 永远为真、每次启动都重写一遍文件。
 * @param file - store path.
 * @returns how many rows were migrated.
 */
function migrateHistoryCost(file) {
	let text = "";
	try {
		text = readFileSync(file, "utf8");
	} catch (error) {
	...
	for (const line of text.split("\n")) {
		...
		const version = raw !== null && typeof raw === "object" && typeof raw.v === "number" && Number.isFinite(raw.v) ? raw.v : 1;
		if (version < COST_VERSION) stale += 1;
	}
	if (stale === 0) return 0;
	const { records } = loadRecords(file);
	const tmp = `${file}.tmp`;
...
function createRoutes(ctx, options) {
	...
	/** 把宿主当前生效的汇率告诉浏览器半，保证两端换算出同一个数字。 */
	const handleRate = (req, res) => {
		send(res, { ok: true, value: { usdCny: USD_CNY_RATE, source: USD_CNY_RATE === DEFAULT_USD_CNY_RATE ? "fallback" : "live" } });
	};
	return { handleBalance, handleHistoryPost, handleSummary, handleRate };
}

/**
 * Plugin body: register the projection and the four HTTP routes.
 * @param ctx - host plugin context.
 */
async function apply(ctx) {
	// 汇率必须在任何历史读写之前定下来（存量迁移与价格表都依赖它），
	// 否则同一批数据会被两种汇率换算。取不到就静默回退，不阻塞启动。
	await initExchangeRate((message) => ctx.logger?.info(message));
	...
	// valley rate.
	try {
		// 口径迁移必须排在两个 repair 之前：repair* 会用当前价格表重算 cost，
		// 而当前价格表是美元；若先 repair，人民币存量会被按美元价重算，
		// 金额就废了。迁移之后再 repair，输入输出都是美元，口径一致。
		const migrated = migrateHistoryCost(historyPath(home));
		if (migrated > 0) ctx.logger?.info(`ui-usage: migrated ${migrated} history rows from CNY to USD pricing`);
	...
	ctx.effect(() => ctx.webServer.register({ kind: "exact", path: "/dsh-client-ui-usage/rate", handler: routes.handleRate }), "ui-usage: rate route");
}

export { COST_VERSION, DEFAULT_USD_CNY_RATE, aggregateSummary, appendRecords, apply, createRoutes, historyPath, inject, initExchangeRate, loadRecords, migrateCost, migrateHistoryCost, name, parseRecord, readProjectionModels, readSessionTitles, repairUnknownModels, repairWeekendPeak, shortSessionId, turnModelsProjectionDefinition };
