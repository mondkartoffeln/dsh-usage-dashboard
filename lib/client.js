window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ── 货币口径（唯一权威，改动前先读）──────────────────────────────
		// 官方定价页给出的是【人民币】单价（api-docs.deepseek.com/zh-cn/
		// quick_start/pricing），而本插件面向按【美元】计费的账号。为了让
		// "成本"和"余额"是同一个货币，这里做且只做一次显式换算：
		//   官方单价(元) --÷ USD_CNY_RATE--> 美元单价 --> 成本($) --> 余额($)
		// 其余任何地方都不得再换算，否则口径漂移、数字对不上。
		//
		// 汇率以【宿主】为准：宿主在启动时确定（环境变量 → 自动获取 → 兜底），
		// 浏览器通过 /dsh-client-ui-usage/rate 读同一个值，避免两端算出不同数字。
		// 这里的 6.7769 只是宿主还没答复前的初值，以及拿不到时的兜底。
		/** 人民币/美元汇率（1 美元 = 多少元）兜底值；实际以宿主返回为准。 */
		const FALLBACK_USD_CNY_RATE = 6.7769;
		/** 当前生效汇率（宿主答复后更新，并会据此重建价格表）。 */
		let USD_CNY_RATE = FALLBACK_USD_CNY_RATE;
		/** 成本显示货币符号（换算后即美元）。 */
		const COST_UNIT = "$";
		/**
		 * 把一行人民币单价换算成美元单价。
		 * @param row - `{input, cacheRead, output}`，单位元/百万 tokens。
		 * @returns 同形状的美元单价。
		 */
		function usdRow(row) {
			return {
				input: row.input / USD_CNY_RATE,
				cacheRead: row.cacheRead / USD_CNY_RATE,
				output: row.output / USD_CNY_RATE
			};
		}
		...
		const DEFAULT_PRICES = {
			...
			// 官方现行模型名。缺这个键会让新会话回退到 default（= v4-pro 价），
			// 实测高估 3 倍 —— 这是"金额不对"的第二个原因。必须与 lib/index.js 同步。
			"deepseek-flash": {
				peak: { input: 3.0, cacheRead: 0.1, output: 9.0 },
				valley: { input: 1.5, cacheRead: 0.05, output: 4.5 }
			},
			// 旧模型名，官方仍按 Flash 价计费（定价页脚注 1），保留以免历史记录掉进默认价。
			"deepseek-v4-flash": {
				peak: { input: 3.0, cacheRead: 0.1, output: 9.0 },
				valley: { input: 1.5, cacheRead: 0.05, output: 4.5 }
			}
		...
		/**
		 * Built-in rate table，已换算为美元：known models plus a fallback row
		 * for unlisted ids. 上面 DEFAULT_* 的原始数字是官方人民币价（便于
		 * 与定价页逐项核对），进表时统一 ÷ USD_CNY_RATE。
		 * 用函数动态构建：汇率由宿主在启动时确定，浏览器拿到后就地重建，
		 * 因此不需要把 USD_CNY_RATE 提前冻结成常量。
		 */
		function buildPriceTable() {
			return {
				default: { peak: usdRow(DEFAULT_ROW.peak), valley: usdRow(DEFAULT_ROW.valley) },
				...Object.fromEntries(Object.entries(DEFAULT_PRICES).map(([model, rows]) => [
					model,
					{ peak: usdRow(rows.peak), valley: usdRow(rows.valley) }
				]))
			};
		}
		/** 当前价格表；汇率更新后重建（见 apply 启动逻辑）。 */
		let PRICE_TABLE = buildPriceTable();
		// ── 颜色语义（唯一权威，改动前先读这里）──────────────────────────
		// PEAK_COLOR / VALLEY_COLOR 只表示"峰时 / 谷时"这一件事：
		// 时段圆点、分时柱、峰谷对比卡、记录徽章、tooltip 的峰/谷两行。
		// 任何"中性强调"（趋势图、日历选中、有数据标记、余额可用、悬停）
		// 一律用 ACCENT_COLOR，禁止借用峰谷色，否则同一屏里橙色会同时
		// 意味着"峰时"和"普通高亮"，用户会读成峰谷颠倒。
		const PEAK_COLOR = "#f97316"; // 橙黄 = 峰时
		const VALLEY_COLOR = "#10b981"; // 绿 = 谷时
		/** 中性强调色（= PALETTE[0]）；与峰/谷语义无关。 */
		const ACCENT_COLOR = "#4f7cff";
		/** 中性强调色的浅色变体，用于同一图表里的次级刻度（如最低值）。 */
		const ACCENT_COLOR_SOFT = "#93b4ff";
		/** "缓存写"在 Token 结构图里的独立色；不借用峰谷色。 */
		const CACHE_WRITE_COLOR = "#8b5cf6";
		/** Chart palette. */
		...
		// 货币口径见文件顶部「货币口径」块（USD_CNY_RATE / COST_UNIT / usdRow）；
		// 这里只放显示格式化，不再定义换算常量，避免两处口径各说各话。
		function fmtCost(value) {
			const v = num(value);
			if (v <= 0) return `${COST_UNIT}0`;
			if (v >= 100) return `${COST_UNIT}${v.toFixed(2)}`;
			if (v >= 0.01) return `${COST_UNIT}${v.toFixed(4)}`;
			return `${COST_UNIT}${v.toFixed(6)}`;
		}

		function fmtCostShort(value) {
			const v = num(value);
			if (v <= 0) return `${COST_UNIT}0`;
			if (v >= 1) return `${COST_UNIT}${v.toFixed(2)}`;
			return `${COST_UNIT}${v.toFixed(3)}`;
		}
		...
		/**
		 * 把一个 Chat 节点规范化成取数用的三个字段。
		 *
		 * 0.1.5 里同时存在两套 assistant 节点形状，历史踩坑记录：
		 *  A. `legacy.nodes` 的元素是扁平的 AssistantMessageNode
		 *     —— `kind:"assistant"`、`usage`/`turn`/`step`/`time` 都在节点顶层
		 *     （dsh-client-ui-conversation/lib/types/client/contract/records.d.ts:249 的联合类型）。
		 *  B. `ChatSnapshot.nodes` store 里的节点是 ChatConversationViewNode
		 *     —— `kind:"assistant-step"`，数据在 `node.data` 下
		 *     （dsh-client-ui-chat/.../conversation-nodes/assistant.d.ts:7）。
		 * 两套都认，避免再因为换了数据源而整条链路归零。
		 * @param node - 待规范的节点。
		 * @returns `{kind, usage, turn, step, time}`；非 assistant 节点 usage 为 undefined。
		 */
		function normalizeNode(node) {
			if (!node || typeof node !== "object") return null;
			if (node.kind !== "assistant" && node.kind !== "assistant-step") return null;
			// A 形状：扁平；B 形状：数据在 data 下。两者都套一遍再回落。
			const d = node.data !== void 0 && node.data !== null && typeof node.data === "object" ? node.data : node;
			const usage = d.usage ?? node.usage;
			if (usage === void 0 || usage === null || typeof usage !== "object") return null;
			return {
				kind: "assistant",
				usage,
				turn: typeof d.turn === "number" ? d.turn : (typeof node.turn === "number" ? node.turn : 0),
				step: typeof d.step === "number" ? d.step : (typeof node.step === "number" ? node.step : 0),
				time: typeof d.time === "number" ? d.time : (typeof node.time === "number" ? node.time : void 0)
			};
		}
		function computeCosts(nodes, fallbackModel, table, modelsByStep) {
			...
			for (const node of nodes ?? []) {
				const info = normalizeNode(node);
				if (info === null) continue;
				const usage = info.usage;
				const stepKey = `${info.turn}:${info.step}`;
				...
				const price = priceAt(table, stepModel, typeof info.time === "number" ? info.time : Date.now());
				const input = num(usage.inputTokens);
				const cacheRead = num(usage.cacheReadTokens);
				const cacheWrite = num(usage.cacheWriteTokens);
				const output = num(usage.outputTokens);
				...
				entry.cost += cost;
				if (typeof info.time === "number" && info.time < entry.time) entry.time = info.time;
		...
		function Sparkline({ values, width, height, peakFill, valleyFill }) {
			...
			return jsx("svg", {
				...
				children: [
					...
					jsx("circle", { key: "peak", cx: peak.x.toFixed(1), cy: peak.y.toFixed(1), r: 1.8, fill: peakFill ?? ACCENT_COLOR }),
					jsx("circle", { key: "valley", cx: valley.x.toFixed(1), cy: valley.y.toFixed(1), r: 1.8, fill: valleyFill ?? ACCENT_COLOR_SOFT })
				]
			});
		}
			...
			return jsxs("div", { style: { position: "relative" }, onMouseLeave: () => setHover(null), children: [
				jsx("svg", { viewBox: `0 0 ${W} ${H}`, style: { width: "100%", height: "auto", display: "block", minHeight: 90 }, onMouseMove: onMove, children: [
					jsx("defs", { children: jsx("linearGradient", { id: "usageAreaGrad", x1: "0", y1: "0", x2: "0", y2: "1", children: [
						jsx("stop", { offset: "0%", stopColor: ACCENT_COLOR, stopOpacity: 0.26 }),
						jsx("stop", { offset: "100%", stopColor: ACCENT_COLOR, stopOpacity: 0.02 })
					] }) }),
					ticks.map((tick) => {
					...
					jsx("polyline", { points, fill: "none", stroke: ACCENT_COLOR, strokeWidth: 2, strokeLinejoin: "round", strokeLinecap: "round" }),
					buckets.map((b, i) => {
						...
						return jsx("circle", {
							...
							cy: y(b.cost),
							r: hover === i ? 3.6 : 2,
							fill: hover === i ? ACCENT_COLOR : "var(--dsw-alias-bg-base)",
							stroke: ACCENT_COLOR,
							strokeWidth: 1.4,
		...
		function TokenDonut({ tokens, tr }) {
			const parts = [
				{ key: "input", label: tr("price.input"), value: num(tokens?.input), color: ACCENT_COLOR },
				{ key: "cacheRead", label: tr("price.cache"), value: num(tokens?.cacheRead), color: "#06b6d4" },
				{ key: "cacheWrite", label: tr("token.cacheWrite"), value: num(tokens?.cacheWrite), color: CACHE_WRITE_COLOR },
				{ key: "output", label: tr("price.output"), value: num(tokens?.output), color: "#ec4899" }
			];
		...
		const styles = {
			...
			calDaySelected: { background: ACCENT_COLOR, color: "#fff", fontWeight: 700 },
			...
			calDayHasData: { background: "color-mix(in srgb, " + ACCENT_COLOR + " 16%, transparent)", color: ACCENT_COLOR, fontWeight: 600 },
		...
		// ────────────────────────────────────────────────────────────────
		/**
		 * 余额金额的显示前缀。接口的 `currency` 是货币代码（USD / CNY），
		 * 映射成符号是为了好看；未知代码原样保留并留一个空格，绝不猜成 ¥。
		 * @param {string} currency - 接口返回的 currency 字段。
		 * @returns 形如 "$" / "CNY " 的前缀。
		 */
		...
		/**
		 * 优选美元条目：账号可能同时返回 USD 与 CNY 两条，美元是本插件的
		 * 计价口径，因此优先取 USD；没有 USD 时退回第一条。
		 * @param {object[]} infos - 接口的 balance_infos 数组。
		 * @returns 选中的条目，或 undefined。
		 */
		function pickBalance(infos) {
			if (!Array.isArray(infos) || infos.length === 0) return void 0;
			return infos.find((info) => info !== null && info !== void 0 && info.currency === "USD") ?? infos[0];
		}
			...
			return jsx("div", { style: { ...styles.card, gridColumn: "span 12", flexDirection: "row", alignItems: "center", gap: 12, padding: "8px 12px", flexWrap: "wrap" }, children: [
				...
				balanceError !== null && balanceError !== void 0
					...
					: balance === null || row === void 0
						...
						: [
							jsx("span", { key: "total", style: { color: "var(--dsw-alias-label-primary)", fontSize: 19, fontWeight: 700, fontVariantNumeric: "tabular-nums", lineHeight: "24px" }, children: `${currencyPrefix(row.currency)}${row.total_balance ?? "0"}` }),
							balance.is_available === false
								...
								: jsx("span", { key: "badge", style: { ...styles.badge, color: ACCENT_COLOR, border: `1px solid ${ACCENT_COLOR}` }, children: "●" }),
							jsx("span", { key: "split", style: { color: "var(--dsw-alias-label-tertiary)", fontSize: 11 }, children: `${tr("balance.topped")} ${currencyPrefix(row.currency)}${row.topped_up_balance ?? "0"} · ${tr("balance.granted")} ${currencyPrefix(row.currency)}${row.granted_balance ?? "0"}` }),
							balanceAt > 0
										...
										days.map((cell, i) => {
											...
											return jsx("button", {
												...
												children: jsxs("span", { style: styles.calDayInner, children: [
													...
													jsx("span", { style: { width: 3, height: 3, borderRadius: "50%", background: hasData && !selected ? ACCENT_COLOR : "transparent", marginTop: 1 } })
												] })
											});
										})
								] }),
		...
		function UsageDock(props) {
			...
			// 节点必须从 `useChat` 取：Chat 快照的 `legacy.nodes` 才是
			// ConversationNode[]（dsh-client-ui-chat/lib/types/client/contract/
			// snapshot.d.ts:66-67）。`useSession` 绑定的是 session 包的
			// `binding.session`（dsh-client-ui-session/lib/client.js:66），那个快照
			// 里根本没有 `nodes` 字段，所以 `snapshot?.nodes` 恒为 undefined，
			// computeCosts 永远拿到空数组 —— 坞条金额恒为 $0 就是这个原因。
			// 官方 StatsPills 也是读 legacy.nodes（dsh-client-ui-chat/lib/client.js:4080）。
			// `useChat` 由 chat 包以 hooks:["chat"] 注册；万一某个组合没注入它，
			// 就退回 useSession（只会得到空数组，不会抛错）。
			const useNodes = typeof props.useChat === "function" ? props.useChat : useSession;
			const nodes = useNodes((snapshot) => snapshot?.legacy?.nodes ?? snapshot?.nodes) ?? [];
			...
			// ── 本会话累计成本的权威来源 ──────────────────────────────────
			// 坞条的「会话累计」必须用宿主算好的数字，不能让浏览器自己按
			// "每步模型"再算一遍：混合模式（同一会话里 flash 与 pro 交替）下，
			// 浏览器只有 modelSelection（只能表达"一个"模型），拿不到每步真值时
			// 会整体回退到 default(v4-pro) 价 —— 实测把 $0.5488 算成 $1.83，高 3 倍。
			// 历史记录里每条都带宿主写入的 cost，与仪表盘同源，混合模式也正确。
			// 折线图/单轮/时段仍用节点（那些只需要 token 数与轮次，不依赖单价）。
			...
			useEffect(() => {
				...
				params.set("since", "0");
				params.set("limit", "1");
				params.set("turns", "200"); // 覆盖本会话全部轮次，避免长会话被截断
				params.set("session", sessionId);
				const load = () => {
					fetch(`${API_BASE}/history/summary?${params.toString()}`)
						.then((response) => response.json())
						.then((payload) => {
							if (!live || payload?.ok !== true) return;
							const value = payload.value;
							if (value === void 0 || value === null) return;
							const total = num(value.peak?.cost) + num(value.valley?.cost);
							// 该会话还没有任何入库记录时保持 null，让坞条退回节点计算。
							setSessionTotalCost(num(value.inRange) > 0 ? total : null);
						})
						.catch(() => {});
				};
				load();
				// 不要依赖 `open`：它在下面才用 const 定义，放进依赖数组会在
				// 渲染期触发暂时性死区（ReferenceError），整个坞条直接崩掉。
			 ...
			 * 当前会话实际使用的模型。
			 *
			 * 取自 `modelSelection` 投影：`next` 是下一次请求将使用的选择（未消费的
			 * 用户改动），没有时回落到 `lastUsed`（最近一次已记录请求实际用掉的）。
			 * 这正是官方 ui-model-selection 的读法（dsh-client-ui-model-selection/
			 * lib/client.js:225-229），也是 0.1.5 里唯一还存在的"本会话模型"来源。
			 * `useProjection` 读取的键是宿主 `wire` 过的 client-visible 单元，
			 * 所以这里只在宿主那一半修好 `wire` 之后才有值。
			 */
			const model = useMemo(() => {
				const selection = modelSelection?.next ?? modelSelection?.lastUsed;
				return typeof selection?.model === "string" && selection.model.length > 0 ? selection.model : null;
			}, [modelSelection]);
			...
			useEffect(() => {
				...
				const records = [];
				for (const node of nodes ?? []) {
					// 与 computeCosts 共用同一套形状兼容（扁平 AssistantMessageNode
					// 与嵌套 assistant-step 都认），见 normalizeNode 的说明。
					const info = normalizeNode(node);
					if (info === null) continue;
					const usage = info.usage;
					const turn = info.turn;
					...
					// Prefer the durable per-step projection; fall back to the session's
					// `modelSelection` projection. Never post while the model is still
					// unknown — the effect re-runs when either source resolves, and the
					// host upserts corrections over previously posted `default` rows.
					...
					const time = typeof info.time === "number" ? info.time : Date.now();
					const price = priceAt(PRICE_TABLE, known, time);
					records.push({
						// v: 2 = 本记录的成本按【美元】计价。v1 是旧的人民币计价，
						// 宿主侧 parseRecord 会按汇率把 v1 一次性换成美元并升到 v2。
						// 改价格表口径时必须同步改这个版本号，否则历史/新记录会双重换算。
						v: 2,
						sessionId,
						turn,
						step,
			...
			const projTokens = tokenUsage
				...
				: 0;
			// 优先用宿主历史里的权威累计（与仪表盘同源，混合模式也正确）；
			// 只有该会话还没入库任何记录时才退回"节点自算 + 按投影 token 等比放大"。
			let sessionCost = stats.total;
			let approx = false;
			if (sessionTotalCost !== null) {
				sessionCost = sessionTotalCost;
			} else if (stats.nodeTokens > 0 && projTokens > stats.nodeTokens) {
			...
			const balanceRow = balance !== null && balance !== void 0 && Array.isArray(balance.balance_infos)
				? pickBalance(balance.balance_infos) ?? null
				: null;
				...
				children: [
					...
					jsx("span", {
						...
						children: [
							...
							balanceError !== null && balanceError !== void 0
								...
								: jsx("span", { key: "bv", style: { ...styles.num, fontWeight: 600 }, children: balanceRow !== null ? `${currencyPrefix(balanceRow.currency)}${balanceRow.total_balance ?? "0"}` : "…" }),
							jsx("span", {
								key: "br",
					...
					stats.turns.length >= 2
						? jsx(Sparkline, {
								...
								width: 56,
								height: 14,
								peakFill: ACCENT_COLOR,
								valleyFill: ACCENT_COLOR_SOFT
							})
						: null,
		...
		function apply(ctx) {
			// 汇率以宿主为准：宿主在启动时确定（环境变量 → 自动获取 → 兜底），
			// 这里读同一个值并重建价格表，避免两端换算出不同数字。
			// 失败保持 FALLBACK_USD_CNY_RATE，静默降级，不阻塞插件。
			fetch(`${API_BASE}/rate`)
				.then((response) => response.json())
				.then((payload) => {
					const rate = Number(payload?.value?.usdCny);
					if (payload?.ok === true && Number.isFinite(rate) && rate > 0) {
						USD_CNY_RATE = rate;
						PRICE_TABLE = buildPriceTable();
						ctx.logger?.info?.(`ui-usage: 汇率来自宿主 = ${rate}`);
					}
				})
				.catch(() => {});
			...
			//
			// 这里不再注入 `readModel`：它过去走 `connection.api.sessions.models()`，
			// 而 0.1.5 的 `ConnectionHandle` 只暴露 isLoopback/generation/state/rpc/
			// reconnect/registerGenerationSource/start，没有 `api`（dsh-client-connection/
			// lib/types/client/index.d.ts:68-98），那段代码永远走进 catch 返回 null。
			// 现在模型改从 `modelSelection` 投影读取，不再需要宿主侧回调。
